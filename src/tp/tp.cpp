...
void Tp::run(int argc, char *argv[])
{
    if (argc < 2) help();
...
bool Tp::mkDir(const string & remotePath)
{
    return newVF("", remotePath, Msg::FT_DIR);
}


bool Tp::newVF(const string & localPath, const string & remotePath, Msg::FileType type)
{
    if(type == Msg::FT_FILE)
    {
...
bool Tp::rmVF(const string & remotePath)
{
...
bool Tp::lsVF(const string & remotePath)
{
...
bool Tp::getVF(const string & remotePath, const string & localPath)
{
    return true;
}

bool Tp::mvVF(const string & remoteSrcPath, const string & remoteDstPath)
{
